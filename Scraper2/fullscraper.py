...
from items import WebPageItem, WebPolicyItem
...
class generalSpider2(scrapy.Spider):
  ...
  def scrape_interesting_links(self, response):
    ...
    for item in response.css('a'):
      ...
      if len(item.css('a::text').re(r'.rivacy')) != 0: 
        ...
        self.privacy_item = privacy_item
      ...
      for index,tag in enumerate(lucky_tag.css('*')[first_h1_tag_index:]):
        ...
        if '<script' not in tag.get() and '<style' not in tag.get() and '<option' not in tag.get():
          ...
          if re.search(regex, str(tag.css('::text').get()).strip()):
            print('"LAST UPDATED" text: ', tag.css('::text').get())
            last_updated_entry = []

            if len(last_updated_entry) == 0:
              last_updated_entry.append(str(tag.css('::text').get()).strip())

          regex2 = r'^(.olicy .ffective)'
          # re.search(regex, str(tag.css('::text').get()))
          
          
          # # if len(tag.css(' ::text').re(r'.ast .pdated')) != 0:
          if re.search(regex2, str(tag.css('::text').get()).strip()):
            print('"POLICY EFFECTIVE" text: ', tag.css('::text').get())
  ...
  def parse(self, response):
    ...
    print('\n', '-------------', '\n','PARSE COUNT: ', parse_count)

    # conditions_item = WebPolicyItem()
    # print(conditions_item)
    ...
    if parse_count == 1:    
      ...
      domain = WebPageItem()
      self.domain = domain
      self.domain['origin_url'] = 'amazon.com'
      self.domain['link_dict'] = {'privacy': {}, 'terms': {}, 'conditions': {}}
      ...
      print('NEXT LINKSS:',next_links)

      print('\n', 'OUTER OBJECT:', self.domain)
    ...
    if parse_count >= 2:
      ...
      if parse_count == (len(self.next_links) + 1):
          ...
          # relevant_strings = ['privacy_item', 'terms_item', 'conditions_item']

          # if hasattr(self,'privacy_item'):
          #   print('Privacy item stored to self object')
          # if hasattr(self,'terms_item') != 1:
          #   print('NO terms item stored to self object')

          # for index,item in enumerate(relevant_strings):
          #   if hasattr(self, item):
          #     self.domain['link_dict'][index]


          # if 'privacy_item' in globals():
          #   print('Privacy item exists')

          # if 'terms_item' not in self:
          #   print('No terms-item item exists')

          # self.domain['link dict'] =[dict(item) for item in ]
          # self.domain['link dict'] =[dict(self.privacy_item), dict(self.terms_item), dict(self.conditions_item)]
          # yield self.domain
          yield self.domain
